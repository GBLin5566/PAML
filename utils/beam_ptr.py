...
def dup_batch(batch, idx, dup_times):
    ...
    for key in ["input_batch", "target_batch"]:
        new_batch[key] = batch[key][:input_len,
                                    idx:idx + 1].repeat(1, dup_times)

    if "input_ext_vocab_batch" in batch:
        for key in ["input_ext_vocab_batch", "target_ext_vocab_batch"]:
            new_batch[key] = batch[key][:input_len,
                                        idx:idx + 1].repeat(1, dup_times)
        new_batch["article_oovs"] = [batch["article_oovs"][idx]
...
class BeamSearch(object):
    ...
    def beam_search(self, batch):
        ...
        for i in range(batch_size):
            ...
            best_summary = self.beam_search_sample(
                enc_batch,
                enc_padding_mask,
                enc_lens,
            ...
            if config.pointer_gen:
                ...
                decoded_words = [self.lang.index2word[idx]
                                 for idx in output_ids]
    ...
    def beam_search_sample(
        ...
        while steps < config.max_dec_step and len(results) < config.beam_size:
            ...
            s_t_1 = (
                torch.stack(
                    all_state_h, 0).unsqueeze(0), torch.stack(
                    all_state_c, 0).unsqueeze(0))
            ...
            final_dist, s_t, c_t, attn_dist, p_gen, coverage_t = self.model.decoder(
                y_t_1, s_t_1, encoder_outputs, enc_padding_mask, c_t_1, extra_zeros, enc_batch_extend_vocab, coverage_t_1, steps, training=False)

            topk_log_probs, topk_ids = torch.topk(
                final_dist, config.beam_size * 2)
