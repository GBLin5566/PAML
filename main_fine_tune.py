...
def do_learning(model, train_iter, val_iter, iterations):
    ...
    loss, ppl_val, ent_b, bleu_score_b = evaluate(
        model, val_iter, model_name=config.model, ty="test", verbose=False)
    ...
    for val_dial_index in range(num_of_dialog):
        if config.fix_dialnum_train:
            train_iter, val_iter = p.get_balanced_loader(
                persona=per, batch_size=config.batch_size, split='test', fold=val_dial_index, dial_num=config.k_shot)

        else:
            train_iter, val_iter = p.get_data_loader(
                persona=per, batch_size=config.batch_size, split='test', fold=val_dial_index)
        logger = do_learning(
            model,
            train_iter,
            val_iter,
...
if config.fix_dialnum_train:
    config.save_path = config.save_path + \
        '_fix_dialnum_' + str(config.k_shot) + '_'
pickle.dump([fine_tune, iterations], open(
    config.save_path + 'evaluation.p', "wb"))
