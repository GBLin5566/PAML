...
class SoftmaxOutputLayer(OutputLayer):
    ...
    def loss(self, hidden, labels):
        ...
        return F.nll_loss(
            log_probs.view(-1, self.output_size), labels.view(-1))
        ...
        for line in open(config.emb_file).readlines():
            ...
            if(len(sp) == config.emb_dim + 1):
                if sp[0] in vocab.word2index:
                    pre_trained += 1
                    embeddings[vocab.word2index[sp[0]]] = [
...
def evaluate(
    ...
    for j, batch in pbar:
        ...
        if((j < 3 and ty != "test") or ty == "test"):
            ...
            for i in range(len(batch["target_txt"])):
                ...
                ent_b.append(bert.predict_label([sent_beam_search for _ in range(
                    len(batch['persona_txt'][i]))], batch['persona_txt'][i]))

        pbar.set_description(
            "loss:{:.4f} ppl:{:.1f}".format(
                np.mean(l), np.mean(p)))
        if(j > 4 and ty == "train"):
            break
    loss = np.mean(l)
    ppl = np.mean(p)
    ent_b = np.mean(ent_b)
    bleu_score_b = moses_multi_bleu(
        np.array(hyp_b), np.array(ref), lowercase=True)
