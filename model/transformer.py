            ...
            if(config.act):
                ...
                for l in range(self.num_layers):
                    x += self.timing_signal[:,
                                            :inputs.shape[1],
                                            :].type_as(inputs.data)
                    x += self.position_signal[:,
                                              l,
                                              :].unsqueeze(1).repeat(1,
                                                                     inputs.shape[1],
                                                                     1).type_as(inputs.data)
...
class Generator(nn.Module):
    ...
    def forward(
        ...
        if config.pointer_gen:
            ...
            if(beam_search):
                enc_batch_extend_vocab_ = torch.cat(
                    [enc_batch_extend_vocab_[0].unsqueeze(0)] * x.size(0), 0)  # extend for all seq
            logit = torch.log(
                vocab_dist_.scatter_add(
                    2, enc_batch_extend_vocab_, attn_dist_))
            return logit
        else:
...
class Transformer(nn.Module):
    ...
    def decoder_greedy(self, batch):
        ...
        for i in range(config.max_dec_step):
            ...
            decoded_words.append(['<EOS>'if ni.item(
            ) == config.EOS_idx else self.vocab.index2word[ni.item()] for ni in next_word.view(-1)])
    ...
    def score_sentence(self, batch):
        ...
        for i, b in enumerate(enc_batch):
            ...
            encoder_outputs = self.encoder(
                self.embedding(b.unsqueeze(0)), mask_src)
            rank = {}
            for j, c in enumerate(cand_batch[i]):
                ...
                logit = self.generator(
                    ...
                    extra_zeros)
                loss = self.criterion(logit.contiguous(
                ).view(-1, logit.size(-1)), c.unsqueeze(0).contiguous().view(-1))
...
# CONVERTED FROM
# https://github.com/tensorflow/tensor2tensor/blob/master/tensor2tensor/models/research/universal_transformer_util.py#L1062
...
class ACT_basic(nn.Module):
    ...
    def forward(
        ...
        while(((halting_probability < self.threshold) & (n_updates < max_hop)).byte().any()):
            ...
            if(decoding):
                if(step == 0):
                    previous_att_weight = torch.zeros_like(
                        attention_weight).cuda()  # [B, S, src_size]
